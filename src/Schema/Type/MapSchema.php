...
final class MapSchema extends Schema
{
    public function __construct(
        public readonly Schema $values,
        ?LogicalType $logicalType = null,
        array $extraAttributes = [],
    ) {
        parent::__construct(Type::MAP, $logicalType, $extraAttributes);
    }
    ...
    public function getValue(): array
    {
        ...
        $avro[Attribute::VALUES->value] = $this->values->getValue();
    ...
    public function isValidDatum(mixed $datum): bool
    {
        if (\is_array($datum)) {
            foreach ($datum as $k => $v) {
                if (!\is_string($k) || !$this->values->isValidDatum($v)) {
                    return false;
                }
            }
